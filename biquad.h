...
/**
 * @struct Instance structure for a cascade set of biquad filter
 */
struct bq_cas_struct {
    bq_type* coefs;  /**< points to the coefficient array. */ 
    bq_type* delay_line; /**< points to the delay line array. */
    int8_t w0; /**< The delay line index pointing to x[i-1] */
    int8_t stages; /**< Number of biquads */
};

/**
* Initialize the direct form I biquad structure.
* @param bq_inst : cascade biquad structure, which holds the coefficients and delay elements.
* @param num_of_biquads : number of biquads in the cascade filter
* @param coef: a pointer to array of biquad coefficients on the form {b00, b01, b02, a01, a02, b10, b11, b12, a11, a12, b20, ...}. 
*
* @note The coefficients must be of the form where it satisfies the form
 * y[i] = b0*x[i] + b1*x[i-1] + b2*x[i-2] - a1*y[i-1] - a2*y[i-2]
*/
